package psd_count_contigs::psd_count_contigsClient;
...
psd_count_contigs::psd_count_contigsClient

=head1 DESCRIPTION


A KBase module: psd_count_contigs
...
sub new
{
    ...
    my $self = {
	client => psd_count_contigs::psd_count_contigsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a psd_count_contigs.workspace_name
$contigset_id is a psd_count_contigs.contigset_id
$return is a psd_count_contigs.CountContigsResults
...
# Authentication: required
	...
	method => "psd_count_contigs.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "psd_count_contigs.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for psd_count_contigs::psd_count_contigsClient\n";
    }
    if ($sMajor == 0) {
        warn "psd_count_contigs::psd_count_contigsClient version is $svr_version. API subject to change.\n";
    }
}
...
package psd_count_contigs::psd_count_contigsClient::RpcClient;
