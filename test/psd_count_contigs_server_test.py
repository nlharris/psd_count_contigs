...
class psd_count_contigsTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'psd_count_contigs',
        ...
        for nameval in config.items('psd_count_contigs'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = psd_count_contigs(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_psd_count_contigs_" + str(suffix)
